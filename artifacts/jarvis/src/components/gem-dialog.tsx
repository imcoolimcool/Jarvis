...
export function GemDialog({ open, onClose, onCreated }: GemDialogProps) {
  const { t } = useI18n();
  ...
  const handleCreate = useCallback(async () => {
    ...
    setError(null);
    try {
      const res = await fetch('/api/jarvis/conversations/gem', {
        method: 'POST',
      ...
      if (!res.ok) {
        ...
        setError(err?.error ?? `Failed to create gem (${res.status})`);
        return;
      }
          ...
          >
            ...
            <div className="flex items-center justify-between px-4 py-3 border-b border-border/30">
              <div className="flex items-center gap-2">
                ...
                <span className="text-[12px] font-semibold tracking-wide font-rounded">{t('gem.title')}</span>
              </div>
            ...
            <div className="p-4 space-y-3">
              <div>
                <label className="text-[10px] font-mono tracking-widest text-muted-foreground/50">{t('gem.name')}</label>
                <input
                  value={name}
                  onChange={(e) => setName(e.target.value)}
                  placeholder={t('gem.namePlaceholder')}
                  maxLength={120}
              ...
              <div>
                <label className="text-[10px] font-mono tracking-widest text-muted-foreground/50">{t('gem.prompt')}</label>
                <textarea
                  value={prompt}
                  onChange={(e) => setPrompt(e.target.value)}
                  rows={5}
                  placeholder={t('gem.promptPlaceholder')}
                  maxLength={12000}
