...
/** Format a date string into a short relative or absolute time string */
...
/** Group conversations by date range */
...
function SidebarContent({ conversations, activeId, deleting, searchQuery, onNew, onSelect, onDelete, onExport, onSearchChange, onClearAll, onMobileClose }: SidebarContentProps) {
  ...
  return (
    <div className="flex flex-col h-full">
      {/* Header */}
      <div className="p-3 border-b border-border/30 flex-shrink-0 space-y-2">
        ...
        <div className="relative">
          ...
          <input
            ...
            placeholder="Search…"
            className="sidebar-search-input w-full bg-background border border-border text-foreground placeholder:text-muted-foreground/40 font-mono text-[10px] pl-7 pr-2 py-1.5 rounded-md outline-none focus:border-primary/40 focus:ring-1 focus:ring-primary/20 transition-all"
          />
          {searchQuery && (
            <button
      ...
      <div className="flex-1 overflow-y-auto py-2 space-y-3 px-2">
        ...
        {groups.map(group => (
          <div key={group.label}>
            <p className="text-[9px] font-mono text-muted-foreground/40 tracking-widest px-3 mb-1 uppercase">
              {group.label}
            </p>
              ...
              {group.items.map(conv => (
                ...
                >
                  ...
                  <div className="flex-1 min-w-0">
                    <span className="leading-tight line-clamp-2 break-words block pr-5">
                      {conv.title}
                    </span>
                  ...
                  <span
                    ...
                    className="absolute right-6 top-2.5 opacity-0 group-hover:opacity-100 transition-opacity text-muted-foreground/60 hover:text-foreground cursor-pointer"
      ...
      <div className="p-3 border-t border-border/20 flex-shrink-0">
        {conversations.length > 0 && onClearAll ? (
          <button
            onClick={onClearAll}
            className="w-full text-[9px] font-mono text-muted-foreground/40 tracking-widest text-center hover:text-red-400/70 transition-colors py-1"
          >
            CLEAR ALL
          </button>
        ) : (
...
export function ChatSidebar({ activeId, onSelect, onNew, refreshTick, mobileOpen, onMobileClose }: ChatSidebarProps) {
  ...
  // Listen for Ctrl+K to focus search
  useEffect(() => {
  ...
  const handleExport = async (id: string) => {
    try {
      ...
      // Also trigger download as .md file
          ...
          >
            ...
            >
              <div className="flex items-start gap-3">
                ...
                <div>
                  <p className="text-sm font-display font-bold tracking-wide text-foreground">Delete all conversations?</p>
