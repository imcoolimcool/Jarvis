...
export function BuildStudio({ open, onClose, title, initialCommands, onRefreshFiles }: BuildStudioProps) {
  ...
  const doScaffold = async (prompt: string, answers: Record<string, string>, feedbackText: string | null) => {
    ...
    const { response, data } = await apiJson<{ files?: string[]; previewCommand?: string }>('/api/jarvis/build/scaffold', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ workspaceId, prompt, answers, feedback: feedbackText }) });
    if (response.ok) {
      setScaffoldPrompt(''); setLastPrompt(prompt); setLastAnswers(answers); setFeedback('');
      if (data.previewCommand) setPreviewCommand(data.previewCommand);
      setNotice(t('studio.build.starterCreated'));
      await loadFiles();
      await runAutoPipeline(prompt, answers, data.files ?? []);
    } else {
  ...
  const captureScreenshot = async (): Promise<boolean> => {
    if (screenshotBusy) return Boolean(screenshot);
    ...
    if (response.ok && data.dataUrl) { setScreenshot(data.dataUrl); return true; }
  ...
  const runAutoPipeline = async (prompt: string, answers: Record<string, string>, createdFiles: string[]) => {
    ...
    for (let pass = 1; pass <= 2; pass += 1) {
      ...
      const { response, data } = await apiJson<IterateResponse>('/api/jarvis/build/iterate', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ workspaceId, prompt, answers, previewOutput: output, passNumber: pass }) });
      ...
      if (data.filesChanged?.length) {
        ...
        await loadFiles();
        if (pass < 2) { await launchPreview(); await waitMs(1200); captured = await captureScreenshot(); if (captured) addActivity(t('studio.build.activityScreenshot'), 'camera', 1); }
      }
